...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($billing_errors)) {
        if (isset($_SESSION['order_id'])) {
            ...
            $q = "CALL add_order_not_login({$_SESSION['customer_id']}, '$cart_id', $shipping, $discount_price, '$discount_code', $cc_last_four, @total, @oid)";
        ...
        if (isset($order_id, $order_total)) {
            try {
                ...
                if ($charge->paid == 1) {
                    ...
                    $q = "CALL add_charge_not_login('{$charge->id}', $order_id, 'auth_only', $order_total, '$full_response')";
                    $r = mysqli_query($dbc, "$q");
                    $_SESSION['response_code'] = $charge->paid;
                    redirect('?mod=checkout&act=final_not_login');
                } else {
...
include("view/checkout/billing_not_login.php");
get_footer();
